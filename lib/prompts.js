    ...
    .then(({ name }) => {
      ...
      return inquirer
        ...
        .then(({ id, email }) => {
          ...
          // based on the role, prompt further questions
          switch (role) {
            case 'Manager':
...
const promptManager = (employeeData) => {
  ...
  return inquirer
    ...
    .then(({ office }) => {
      // store the data in employeeRoster
...
const promptMoreEmployees = () => {
  return inquirer
    ...
    .then((data) => {
      // if new team member is to be added, recursively call the function
      ...
      // all finished, return all the data
